    ...
    if (pull_data->summary)
      {
        ...
        n = deltas ? g_variant_n_children (deltas) : 0;
        pull_data->summary_has_deltas = n > 0;
        for (i = 0; i < n; i++)
          {
            const char *delta;
            g_autoptr(GVariant) csum_v = NULL;
                                 ...
                                 GError       **error)
{
  ...
  *out_summary_idx = NULL;
  if (out_summary_idx_sig)
    *out_summary_idx_sig = NULL;

  if (!ostree_repo_remote_get_gpg_verify_summary (self, name,
  ...
  /* If the index matches the cache we know it is valid, and either
     the cache has a matching signature, or it had no signture at
