 ...
 * @self: Repo
 * @out_deltas: (out) (element-type utf8) (transfer container): String name of delta index (checksum)
 * @cancellable: Cancellable
 * @error: Error
 *
 * This function synchronously enumerates all static deltas in the
      ...
      if (froms == NULL)
        {
          ...
          if (!glnx_shutil_rm_rf_at (repo->repo_dir_fd, index_path,
                                     cancellable, error))
            return FALSE;
        }
      else
